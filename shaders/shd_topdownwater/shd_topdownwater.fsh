...
void main()
{
    ...
    vec2 uv = fragCoord.xy / iResolution.xy;// (1.2 * iResolution.yy);
    uv *= water_strength;
    ...
    // Načtení barvy ze základní textury
    vec4 texColor = texture2D(gm_BaseTexture, uv / 20. + p / 45.);

    // Pokud je pixel skoro průhledný, nepoužijeme efekt
    if (texColor.a < 0.1) {
        gl_FragColor = texColor; // Žádné zkreslení
    } else {
        // Aplikace efektu
        vec3 col = mix(texColor.rgb, vec3(.7, .7, .9), c);
