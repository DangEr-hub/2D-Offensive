...
if(get_rank(global.player_elo_struct.Previous_elo) < get_rank(global.player_elo_struct.Elo)){
	...
	var py = random_range(camera_get_view_y(CAMERA), camera_get_view_y(CAMERA) + string_width("Rank up!"));
	...
	with(zui_main()){
		with(zui_create(global.GuiW/2 - string_width("Rank up!")/2, string_height("a")/2, objUILabel, -10001)){
			alpha = 1;
			alpha_value = 0;
			color = MAIN_COLOR;
			caption = "Rank up!";
...
if(rank_previous < rank_position){	
