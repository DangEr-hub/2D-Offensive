...
content_title_y = max(75, 64 * global.GUIMultiplier);
...
control_gap = max(32 * global.GUIMultiplier, 48);
...
xp_bar_size = max(64 * global.GUIMultiplier, 80);
...
with(zui_create(xp_bar_x, xp_bar_y, objUIImage)){
	...
	bar_radius = other.xp_bar_size * .5 - max(4 * global.GUIMultiplier, 6);
	bar_thickness = max(4 * global.GUIMultiplier, 6);
}
