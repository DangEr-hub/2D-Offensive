...
with(oEnemy){
	if(Visible == true && State != States.Death){
		if(CanShoot == false && ShootTimer >= global.ItemIndex[#WeaponID[WeaponPositionID], ItemStat.ShootTimer]/1.5){
...
with(oPlayer){
	if(CanShoot == false && ShootTimer >= global.ItemIndex[#global.weapon_id[min(WeaponID, 2)], ItemStat.ShootTimer]/1.5){
