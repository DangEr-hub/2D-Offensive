...
if(global.EnemyCanMove == true){
	...
	switch(State){
		case States.MoveAway:
			if(ReactionTimer <= 0){
				shooting_chance = min(10 / (global.ItemIndex[#WeaponID[WeaponPositionID], ItemStat.ShootTimer]/2) * get_rank_boost(global.player_rating_struct.Enemy_ep[global.player_rating_struct.Current_game]), 100);
...
if(place_meeting(x, y, oFriend)) {
	var Enemy = instance_nearest(x, y, oFriend); // Get nearest Enemy
	if(Enemy.id != id){
		var dir = point_direction(Enemy.x, Enemy.y, x, y); // Direction from Enemy to player
